(define (select-instructions program)
    ...
    (define (select-effs eff*)
        (match eff*
            ...
            [((set! ,var (* ,tri1 ,tri2)) ,eff1* ...)
                (guard (imm64? tri1) (imm64? tri2) )
                ; tri1 和 tri2 都是 int64
                (define uv1 (unique-name 't))
                (define uv2 (unique-name 't))
                (receive (uvs1 seff1*) (select-effs eff1*)
                (values 
                    (cons uv1 (cons uv2 uvs1))
                    (cons (make-begin (list
                        (list 'set! uv1 tri1)
                        (list 'set! uv2 tri2)
                        (list 'set! uv1 (list '* uv1 uv2))
                        (list 'set! var uv1))) seff1*)
                ))]
            [((set! ,var (* ,tri1 ,tri2)) ,eff1* ...)
                (guard (imm64? tri1))
                ; tri1 是 int64
            ...
            [((set! ,var (* ,tri1 ,tri2)) ,eff1* ...)
                (guard (eq? var tri1) (maybe-reg? var) )
                ; var = triv1 且 binop = * 且 var是寄存器
                (receive (uvs1 seff1*) (select-effs eff1*)
                (values 
                    uvs1
                    (cons (list 'set! var (list '* tri1 tri2)) seff1*)
                ))]
            [((set! ,var (* ,tri1 ,tri2)) ,eff1* ...)
                (guard (eq? var tri2) (maybe-reg? var) )
                ; var = triv2 且 binop = * 且 var是寄存器
                (receive (uvs1 seff1*) (select-effs eff1*)
                (values 
                    uvs1
                    (cons (list 'set! var (list '* tri2 tri1)) seff1*)
                ))]
            [((set! ,var (* ,tri1 ,tri2)) ,eff1* ...)
                (guard (maybe-reg? var))
                ; binop = * 且 var是寄存器
            ...
            [((set! ,var (,binop ,tri1 ,tri2)) ,eff1* ...)
                ...
                (values 
                    ...
                    (cons (make-begin (list
                        (list 'set! uv1 tri1)
                        (list 'set! uv2 tri2)
                        (list 'set! uv1 (list binop uv1 uv2))
                        (list 'set! var uv1)
                    )) seff1*)
                ))]
            [((set! ,var (,binop ,tri1 ,tri2)) ,eff1* ...)
                ...
                (values 
                    ...
                    (cons (make-begin (list 
                        ...
                        (list 'set! var uv))) seff1*)
                ))]
            [((set! ,var (,binop ,tri1 ,tri2)) ,eff1* ...)
                (guard (imm64? tri2) (commu? binop))
                ; 后者是int64 且 binop可交换
                (define uv (unique-name 't))
                (receive (uvs1 seff1*) (select-effs eff1*)
                (values 
                    ...
                    (cons (make-begin (list 
                        (list 'set! uv tri2)
                        (list 'set! uv (list binop uv tri1))
                        (list 'set! var uv))) seff1*)
                ))]
            [((set! ,var (,binop ,tri1 ,tri2)) ,eff1* ...)
                (guard (imm64? tri2) (eq? var tri1))
                ; 后者是int64 且 binop不可交换 且 triv1 = var
                (define uv (unique-name 't))
                (receive (uvs1 seff1*) (select-effs eff1*)
                (values 
                    (cons uv uvs1)
            ...
            [((set! ,var (,binop ,tri1 ,tri2)) ,eff1* ...)
                (guard (eq? var tri1) (or (maybe-reg? var) (maybe-reg? tri2) (int32? tri2)))
                ; var = triv1 且 (至少有一者是寄存器 或 triv2是立即数)
                (receive (uvs1 seff1*) (select-effs eff1*)
                (values 
                    uvs1
                    (cons (list 'set! var (list binop tri1 tri2)) seff1*)
                ))]
            [((set! ,var (,binop ,tri1 ,tri2)) ,eff1* ...)
                (guard (eq? var tri2) (commu? binop) (or (maybe-reg? var) (maybe-reg? tri1) (int32? tri1)))
                ; var = triv2 且 (至少有一者是寄存器 或 triv2是立即数) 且 binop可交换
                (receive (uvs1 seff1*) (select-effs eff1*)
                (values 
                    uvs1
                    (cons (list 'set! var (list binop tri2 tri1)) seff1*)
                ))]
            [((set! ,var (,binop ,tri1 ,tri2)) ,eff1* ...)
                ...
                (values 
                    ...
                    (cons (make-begin (list 
                        (list 'set! var tri1)
                        (list 'set! var (list binop var tri2)))) seff1*)
                ))]
            [((set! ,var (,binop ,tri1 ,tri2)) ,eff1* ...)
                ...
                (values 
                    ...
                    (cons (make-begin (list 
                        (list 'set! var triv2)
                        (list 'set! var (list binop var tri1)))) seff1*)
                ))]
            [((set! ,var (,binop ,tri1 ,tri2)) ,eff1* ...)
                (guard (var? var) (triv? tri1) (triv? tri2) (maybe-reg? var) (eq? var tri2))
                ; var是寄存器 且 var 是 triv2 且 binop不可交换
                (define uv (unique-name 't))
                (receive (uvs1 seff1*) (select-effs eff1*)
                (values 
                    (cons uv uvs1)
    ...
    (define (select-pred pred)
        (match pred
            ...
            [(,relop ,tri1 ,tri2) (guard (maybe-reg? tri1) (maybe-reg? tri2))
                (values '() (list relop tri1 tri2))
            ]
            [(,relop ,tri1 ,tri2) (guard (maybe-reg? tri1) (int64? tri2))
                (values '() (list relop tri1 tri2))
            ]
            [(,relop ,tri1 ,tri2) (guard (int64? tri1) (maybe-reg? tri2))
                (values '() (list (rev-relop relop) tri2 tri1))
            ]
            [(,relop ,tri1 ,tri2) (guard (frame-var? tri1) (maybe-reg? tri2))
                (values '() (list relop tri1 tri2))
            ]
            [(,relop ,tri1 ,tri2) (guard (maybe-reg? tri1) (frame-var? tri2))
                (values '() (list (rev-relop relop) tri2 tri1))
            ]
            [(,relop ,tri1 ,tri2) (guard (triv? tri1) (triv? tri2))
                ...
                (values 
                    (list uv)
                    (make-begin (list (list 'set! uv tri2) (list relop tri1 uv)))
                )
            ]
