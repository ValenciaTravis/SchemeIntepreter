...
(define (uncover-frame-conflict program)
    ...
    (define (get-uvars ls)
        (define ret '())
        (define (add elem) (when (uvar? elem) (set! ret (cons elem ret))))
        (seqmap add ls)
        ret
    )
    ...
    (define (uncover-body body)
        ...
        (define (make-effs-set eff* graph set)
            (match eff*
                ...
                [((mset! ,tri1 ,tri2 ,tri3), eff1* ...)
                    ...
                    (when (or (frame-var? tri1) (uvar? tri1)) (set! set1 (set-cons tri1 set1)))
                    (when (or (frame-var? tri2) (uvar? tri2)) (set! set1 (set-cons tri2 set1)))
                    (when (or (frame-var? tri3) (uvar? tri3)) (set! set1 (set-cons tri3 set1)))
                    ; (display (list 'graphaft: graph1)) (newline)
                    (values graph1 set1))
                ]
                [((return-point ,lab ,tail) ,eff1* ...)
                    (receive (graph1 set1) (make-effs-set eff1* graph set)
                    (receive (graph2 set2) (make-tail-set tail graph1 set1)
                    (set! call-live (union call-live set2))
                    (values graph2 set2)))]
            )
        )
        ...
        (match body
            [(locals (,uv* ...) (new-frames (,frame* ...) ,tail)) 
                (receive (graph set) (make-tail-set tail (map (lambda (x) (list x)) uv*) '())
                (list 'locals uv* 
                    (list 'new-frames frame* 
                    (list 'spills (get-uvars call-live) 
                    (list 'frame-conflict (wrapper graph) 
                    (list 'call-live call-live
                    tail))))))
            ]
        )
    )
