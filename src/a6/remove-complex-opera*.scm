...
(define (remove-complex-opera* program)
    ...
    (define (remove-values val*)
        ...
        (emit "remove-values")
        (emit rval*) 
        (values uvs (make-begin (append add (list rval*))))
    )
    ...
    (define (remove-eff eff)
        ...
        (match eff
            ...
            [(set! ,var (,binop ,tri1 ,tri2))  (guard (triv? tri1) (triv? tri2)) 
                (values '() (list 'set! var (list binop tri1 tri2)))]
            [(set! ,var (,binop ,tri1 ,tri2)) (guard (triv? tri1)) 
                (define uv (unique-name 't))
            ...
            [(set! ,var (,binop ,tri1 ,tri2)) (guard (triv? tri2)) 
                (define uv (unique-name 't))
