(language-wrapper
  (lambda (pass-name x)
    ...
    (case pass-name
      [(source verify-scheme remove-complex-opera* flatten-set!)
       `(let ()
            ...
            (let ()
              ...
              (syntax-rules (,frame-pointer-register)
                [(_ ,frame-pointer-register (op xxx n))
                 (begin
        ;           (printf "(set! ~s ~s)\n" (quote ,frame-pointer-register) (quote (op xxx n)))
                   (set! $fp-offset (op $fp-offset n))
                   (set! ,frame-pointer-register (op xxx n)))]
                [(_ x expr)
        ;         (begin (printf "(set! ~s ~s)\n" (quote x) (quote expr)) (set! x (handle-overflow expr)))])))
                    (set! x (handle-overflow expr))])))
          (define-syntax locals
            (syntax-rules ()
              [(_ (x* ...) body) (let ([x* 0] ...) body)]))
