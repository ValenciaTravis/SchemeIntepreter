         ...
         (let ([n (f (tagref tag-vector disp-vector-length ptr))])
           (let ([v (make-vector n)])
             (do ([i 0 (+ i 1)])
               ...
               (vector-set! v i
                 (f (tagref tag-vector
                            (+ disp-vector-data (fxsll i align-shift))
                            ptr))))))]
...
(language-wrapper
  (lambda (pass-name x)
    ...
    (case pass-name
      [(source verify-scheme optimize-direct-call
        remove-anonymous-lambda sanitize-binding-forms)
       `(let ()
       ...
       `(let ()
          ...
          (define-syntax well-known
            (syntax-rules ()
              [(_ (wk ...) body) body]))
          ,x)]
      [(introduce-procedure-primitives optimize-source)
       `(let ()
      ...
      [(uncover-locals)
       `(let ()
          ...
          (define (nop) (void))
          (ptr->datum ,x))]
      [(remove-let verify-uil remove-complex-opera* flatten-set!)
       `(let ()
          (import (except scheme set!))
          (define int64-in-range?
            (let ()
       ...
       `(let ()
          ...
          (call/cc 
            (lambda (k)
              ...
              ,x))
          (ptr->datum ,return-value-register))]
      [(expose-frame-var expose-memory-operands)
       `(let ()
          (import (except scheme set!))
          (define int64-in-range?
            (lambda (x)
          ...
          (call/cc 
            (lambda (k)
              ...
              ,(rewrite-opnds x)))
          (ptr->datum ,return-value-register))]
      [(expose-basic-blocks optimize-jumps)
       `(let ()
          (import (except scheme set!))
          (define int64-in-range?
            (lambda (x)
