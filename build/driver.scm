              ...
              (if (top-level-bound? 'pass-names)
                  (let ([p pass-names])
                    (unless (valid-passes? p)
                      (format-error 'pass-names "invalid pass-names value ~s" p))
                    p)
                  '()))]
      [(p)
       (unless (valid-passes? p)
         (format-error 'compiler-passes "invalid pass list ~s" p))
       (set! passes p)])))
...
(define language-wrapper
  (make-parameter (lambda (x) `(let () (import scheme) ,x))
    (lambda (x)
      (unless (procedure? x)
        (format-error 'language-wrapper "~s is not a procedure" x))
      x)))
      ...
      [(x)
       (set! trace-list
         (cond
           ...
           [else (format-error 'tracer "invalid argument ~s" x)]))])))
  ...
  (make-parameter
    ...
    (lambda (x)
      (cond
        ...
        [(eq? x #f) '()]
        [(and (list? x) (andmap (lambda (x) (memq x (all-pass-names))) x)) x]
        [else (format-error 'timed-passes "invalid pass names ~s"
  ...
  (make-parameter
    ...
    (lambda (x)
      (cond
        ...
        [(eq? x #f) '()]
        [(and (list? x) (andmap (lambda (x) (memq x (all-pass-names))) x)) x]
        [else (format-error 'trusted-passes "invalid pass names ~s"
                (if (list? x) (difference x (all-pass-names)) x))]))))
...
(define game-eval
  (make-parameter interpret
    (lambda (x)
      (unless (procedure? x)
        (format-error 'game-eval "~s is not a procedure" x))
      x)))
  ...
  (make-parameter #f
    (lambda (p)
      (unless (or (not p) (memq p (all-pass-names)))
        (format-error 'starting-pass "unrecognized pass ~s" p))
      p)))
      ...
      ((call/cc
         (lambda (k)
           (with-exception-handler
             (lambda (x)
               (let ([who (condition-who x)]
                     [msg (condition-message x)]
                     [args (condition-irritants x)])
             ...
             (lambda ()
               ...
               (lambda ()
                 (if ordinal
                     (format-error #f "no error from ~s on test ~s" verifier ordinal)
                     (format-error #f "no error from ~s" verifier)))))
       ...
       (let ([v (begin e1 e2 ...)])
         (let ([elapsed (sstats-difference (statistics) before)])
           (values
             v
             (format "~asec, ~amb"
                     (fmt (/ (time-nanosecond (sstats-cpu elapsed)) (expt 10 9)) 2)
        ...
        (lambda (input-expr passes)
          (define exit
            (lambda (input-expr sp)
              (if sp
                  (format-error 'driver "starting pass ~s not found" sp)
