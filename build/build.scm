...
(define-who build-and-run
  ...
  (lambda (input-expr output-string)
    (define src-file (format "~a/~a.s" output-dir file-root))
    (define out-file (format "~a/~a.out" output-dir file-root))
    (define exe-file (format "~a/~a" output-dir file-root))
    (with-output-to-file src-file
      ...
      'replace)
    (unless (= (shell "gcc -m64 -o ~a build/runtime.c ~a > ~a 2>&1" exe-file src-file out-file) 0)
      (printf "========\n")
